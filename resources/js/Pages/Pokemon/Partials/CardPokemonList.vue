<script setup>
  import CardPokemon from '@/Pages/Pokemon/Partials/CardPokemon.vue';
  const { pokemons, limit } = defineProps({
      pokemons: {
        type: Object,
        default: null,
      },
      limit: {
        type: Number,
        default: 0,
      }
  });
</script>

<template>
  <div class="py-5">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
          <h2 class="text-2xl font-extrabold dark:text-white mb-4">Favoritos recientemente: </h2>                                     
          <div v-for="(pokemon, index) in pokemons" class="inline-block w-1/4 mb-2">
            <CardPokemon 
              v-if="index < limit" 
              :key="index"
              :pokemon="pokemon"      
              class="inline-block w-full"            
            />
          </div>
          <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-1.5 mr-1 mb-1 mt-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
            <a href="/favorites/">Ver más</a>
          </button>
        </div>
      </div>
    </div>
  </div>
</template>